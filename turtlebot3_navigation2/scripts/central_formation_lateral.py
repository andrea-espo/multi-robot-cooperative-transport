...
class ImprovedFormationController(Node):
    ...
    def __init__(self):
        ...
        # distanza desiderata LATERALE rispetto al leader
        ...
        # Safety parameters with hysteresis (come nel file originale)
        self.declare_parameter("safety_distance_enter", 0.30)
        self.declare_parameter("safety_distance_exit", 0.40)
        self.declare_parameter("slow_down_distance_enter", 0.55)
        self.declare_parameter("slow_down_distance_exit", 0.65)

        # Avoidance radius (zona di safety attorno al leader in traiettoria)
        # Deve essere > safety_distance_enter ma non troppo grande
        self.declare_parameter("avoid_radius", 0.40)

        # Safe waypoint offsets (waypoint ravvicinato al leader)
        # Waypoint a (distance + safe_back_offset) dietro,
        # e (distance + safe_side_offset) di lato rispetto al leader
        ...
        # Tolerances
        self.declare_parameter("pos_tolerance", 0.12)
        ...
        log_filename = f"{log_dir}/formation_lateral_log_{timestamp}.csv"
        ...
        self.get_logger().info(
            f"Improved formation controller started:\n"
            f"  Leader: {self.leader_ns}, Follower: {self.follower_ns}\n"
            f"  Lateral formation distance: {self.distance:.2f} m\n"
    ...
    def control_loop(self):
        ...
        if self.reposition_active:
            # prima si va al waypoint safe
            x_goal = self.x_safe
            y_goal = self.y_safe
            yaw_target = yaw_L

            # quando il waypoint safe è raggiunto, torna al target di formazione LATERALE
            if math.hypot(x_goal - x_F, y_goal - y_F) < self.pos_tolerance:
                self.reposition_active = False
                self.get_logger().info(
                    "Safe waypoint reached, switching back to lateral formation target"
                )
                # target di formazione laterale a sinistra del leader
                x_goal = x_L - self.distance * sin_L
                y_goal = y_L + self.distance * cos_L
                yaw_target = yaw_L
        else:
            # target di formazione standard: distanza 'distance' a SINISTRA del leader,
            # sulla stessa "ascissa" (nessun offset avanti/indietro)
            x_goal = x_L - self.distance * sin_L
            y_goal = y_L + self.distance * cos_L
            yaw_target = yaw_L
        ...
        if self.current_mode == "SAFETY":
            ...
            if leader_in_front:
                ...
                v = self.k_linear * error_x_follower * 0.3
            ...
            if leader_in_front:
                ...
                if distance_to_target > 0.2:
                    w = self.k_angular * heading_error * 0.8
                else:
                    w = self.k_angular * yaw_error * 0.8
            else:
                v = self.k_linear * error_x_follower * 0.85
        ...
        # Dentro il raggio di evitamento, ulteriore limitazione della velocità lineare
        if distance_to_leader < self.avoid_radius:
            v = min(v, self.v_max * 0.4)
        ...
        # Smoothing (più forte in SAFETY)
        alpha = 0.5 if self.current_mode != "SAFETY" else 0.8
        ...
        # Machine-friendly log line for plotting
        self.get_logger().info(
            "PLOT "
            f"t={t_sec:.3f} "
